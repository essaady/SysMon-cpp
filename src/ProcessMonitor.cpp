...
            //Pour v�rifier que le nom est entre parenth�ses, il faut que le nom soit encadr� par (..)
            //npos indique l'absence de parenth�ses."
                        proc.name = statLine.substr(start + 1, end - start - 1);
                        //Extraire  le nom
                    }
                }
                 // l'�tat de process
                ifstream statusFile("/proc/" + to_string(pid) + "/status");// infos d�tailer
                string line;
                while (getline(statusFile, line)) {
            // condition pour v�rifier e-c-q nous avons arriv�s a la ligne qui contient l'etat (commencer par(State:)
